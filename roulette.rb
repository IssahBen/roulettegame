...
class RouletteGame
  ...
  def get_amount
    ...
    begin
      ...
      return nil unless amount.positive? && amount <= @balance
  ...
  def column
    ...
    begin
      ...
      numbers = (1..36).select { |n| ((n - col) % 3).zero? }
      amount = get_amount
      return unless amount
  ...
  def red_black
    ...
    result = spin
    actual = if RED_NUMBERS.include?(result)
               'r'
             else
               BLACK_NUMBERS.include?(result) ? 'b' : 'g'
             end
    puts "🎡 Result: #{result} (#{if actual == 'r'
                                   'Red'
                                 else
                                   actual == 'b' ? 'Black' : 'Green'
                                 end})"
